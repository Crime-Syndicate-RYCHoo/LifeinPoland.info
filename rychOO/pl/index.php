...
l1-transitional.dtd">
<html><head>
<?php echo "<meta http-equiv=\"Refresh\" content=\"321; URL=http://localhost/pl/\" />"; ?>
...
function fortfiglet()
{
        ...
        mt_srand( (double)time()*12345678 );
        $litera = mt_rand() % $max;

        $buf = sprintf("echo '%c' | figlet -f eftiwall", $litery[$litera]);
        $pipe = popen($buf, "r") or die("Stream error -> $buf\n");
        $buf = "fortune jpilsudski";
